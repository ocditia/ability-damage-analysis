...
class MagicAbils {
  ...
  timeStrike(type) {
    ...
    const dmg = this.hit.computeOnHit(
      fixed,
      variable,
      SET_INS.preciseRank,
      SET_INS.equilibriumRank,
      true,
    );
    ...
    const onHitAvg = [
      dmg[0][0] + Math.floor(0.5 * dmg[0][1]),
      dmg[1][0] + Math.floor(0.5 * dmg[1][1]),
    ];
    const fcritChance = this.fcrit.computeFCritChance(
      channelers,
      SET_INS.gconc,
    );
    const fcritAvg = [
      this.fcrit.computeFCritDmg(dmg[0][0], dmg[0][1], channelers),
      this.fcrit.computeFCritDmg(dmg[1][0], dmg[1][1], channelers),
    ];
    ...
    if (this.output === "Average") {
      regular =
        Math.floor(onNpc[0] * (1 - fcritChance)) +
        Math.floor(onNpcFCrit[0] * fcritChance);
      sun =
        Math.floor(onNpc[1] * (1 - fcritChance)) +
        Math.floor(onNpcFCrit[1] * fcritChance);
      meta =
        Math.floor(onNpc[2] * (1 - fcritChance)) +
        Math.floor(onNpcFCrit[2] * fcritChance);
    } else {
      regular = onNpc[0];
      sun = onNpc[1];
      meta = onNpc[2];

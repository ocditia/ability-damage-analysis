...
function calc_boosted_ad(settings, dmgObject) {
    ...
    if (abils[settings['ability']]['main style'] === 'magic') {
        // inq staff
        if (settings['weapon type'] === 'two-hand' && settings['two-hand weapon'] === 'inquisitor staff') {
            boosted_AD = Math.floor(boosted_AD * 1.125);
        }

        // inq staff upgraded
        else if (settings['weapon type'] === 'two-hand' && settings['two-hand weapon'] === 'inquisitor staff+') {
    ...
    if (abils[settings['ability']]['main style'] === 'melee') {
        // terrasaur maul
        if (settings['weapon type'] === 'two-hand' && settings['two-hand weapon'] === 'terrasaur maul') {
            boosted_AD = Math.floor(boosted_AD * 1.125);
        }

        // terrasaur maul upgraded
        else if (settings['weapon type'] === 'two-hand' && settings['two-hand weapon'] === 'terrasaur maul+') {
            boosted_AD = Math.floor(boosted_AD * 1.175);
        }
    ...
    if (abils[settings['ability']]['main style'] === 'ranged') {
        ...
        // hex bow upgraded
        else if (settings['weapon type'] === 'two-hand' && settings['two-hand weapon'] === 'hexhunter bow+') {
            boosted_AD = Math.floor(boosted_AD * 1.175);
        }
...
function ability_specific_effects(settings, dmgObject) {
    ...
    if (abils[settings['ability']]['main style'] === 'magic') {
        ...
        if (song_of_destruction_effects.contains(abils[settings['ability']]['ability classification'])) {
        ...
        // wrack bound
        if (settings['ability'] === 'wrack' && (settings['target disability'] === 'stunned' || settings['target disability'] === 'bound' || settings['target disability'] === 'stunned and bound')) {
            dmgObject[boosted_AD] = Math.floor(dmgObject[boosted_AD] * 1.3);
        }

        // wrack and ruin bound
        if (settings['ability'] === 'wrack and ruin' && (settings['target disability'] === 'stunned' || settings['target disability'] === 'bound' || settings['target disability'] === 'stunned and bound')) {
            dmgObject[boosted_AD] = Math.floor(dmgObject[boosted_AD] * 1.6);
        }

        // greater chain half damage
        gchain_not_halved = ['bleed', 'burn', 'dot'];
        if (gchain_not_halved.contains(abils[settings['ability']][['ability classification']] === false)) {
    ...
    if (abils[settings['ability']]['main style'] === 'melee') {
        // slice bound
        if (settings['ability'] === 'slice' && (settings['target disability'] === 'stunned' || settings['target disability'] === 'bound' || settings['target disability'] === 'stunned and bound')) {
            dmgObject[boosted_AD] = Math.floor(dmgObject[boosted_AD] * 1.4);
        }
    ...
    if (abils[settings['ability']]['main style'] === 'ranged') {
        // piercing shot bound
        if (settings['ability'] === 'piercing shot' && (settings['target disability'] === 'stunned' || settings['target disability'] === 'bound' || settings['target disability'] === 'stunned and bound')) {
            dmgObject[boosted_AD] = Math.floor(dmgObject[boosted_AD] * 1.3);
        }
    }
    ...
    if (abils[settings['ability']]['main style'] === 'necromancy') {
        ...
        if (settings['death spark'] === true && settings['ability'] === 'necromancy auto') {
            dmgObject[boosted_AD] = Math.floor(dmgObject[boosted_AD] * 1.5);
        }

        // living dead - finger of death
        if (settings['living dead'] === true && settings['ability'] === 'finger of death') {
            dmgObject[boosted_AD] = Math.floor(dmgObject[boosted_AD] * 2);
        }

        // skeleton warrior stacks
        if (settings['ability'] === 'skeleton warrior auto') {
            dmgObject[boosted_AD] = Math.floor(dmgObject[boosted_AD] * (1 + 0.03 * settings['skeleton warrior stacks']));
        }
    }
}
...
function calc_style_specific(settings, dmgObject) {
    if (abils[settings['ability']]['main style'] === 'ranged') {
        ...
        if (settings['ammunition'] === 'bane bolts' || settings['ammunition'] === 'bane arrows') {
            if (settings['ability'] === 'ranged main-hand auto' || settings['ability'] === 'ranged two-hand auto' || settings['ability'] === 'ranged off-hand auto') {
        ...
        // jas bane ammo
        if (settings['ammunition'] === 'jas dragon bane arrow' || settings['ammunition'] === 'jas demon bane arrow') {
        ...
        if (settings['ammo slot'] === 'pernix quiver' && settings['target hp percent'] <= 25) {
            dmgObject['var hit'] = Math.floor(dmgObject['var hit'] + 0.04 * (dmgObject['min hit'] + dmgObject['var hit']));
        }
    }
}
...
function calc_precise(settings) {
    // calculate precise
    let max_hit = settings['min hit'] + setting['var hit'];
...
function calc_additive_boosts(settings) {
    ...
    let void_pieces = 0;
    const void_chest_pieces = ['void knight top', 'superior void knight top', 'elite void knight top', 'superior elite void knight top'];
    if (void_chest_pieces.includes(settings['chest'])) {
        void_pieces += 1;
    }
    const void_legs_pieces = ['void knight robe', 'superior void knight robe', 'elite void knight robe', 'superior elite void knight robe'];
    ...
    if (settings['enduring ruin - hit'] === 'regular' && abils[settings['ability']]['main style'] === 'melee') {
        boost += 0.1;
    }
    else if (settings['enduring ruin - hit'] === 'enhanced' && abils[settings['ability']]['main style'] === 'melee') {
        boost += 0.16;
    }

    // needle strike next abil boost if style is ranged
    if ((settings['needle strike'] === true || settings['needle strike'] === 'fleeting') && abils[settings['ability']]['main style'] === 'ranged') {
        boost += 0.07;
    }
...
function calc_prayer(settings) {
    ...
    if (abils[settings['ability']]['main style'] === prayers[settings['prayer']]['style']) {
        ...
        if (settings['necklace'] === 'amulet of zealots' && prayers[settings['prayer']]['category'] in ['single-stat boosting', 'leech curse']) {
            boost += 0.1
        }
    }
...
function calc_multiplicative_shared_buffs(settings) {
    ...
    // apply revenge
    if (abils[settings['ability']]['main style'] === 'main-hand' && weapons[settings['off-hand weapon']]['weapon type'] in ['shield', 'defender']) {
...
function calc_multiplicative_pve_buffs(settings) {
    ...
    else if (settings['guard house'] === 'tier 1 undead' || settings['guardhouse'] === 'tier 3 undead') {
...
function calc_bonus_damage(settings) {
    ...
    if (abils[settings['ability']]['main style'] === 'melee') {
        ...
        if ((settings['off-hand weapon'] === 'dark sliver of leng' || settings['off-hand weapon'] === 'dark ice sliver') 
...
function calc_core(settings, dmgObject) {
    for (let i=0; i<dmgObject['damage list'].length; i++) {
        // berserker's fury
        dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * (1 + settings['berserkers fury']/100));
        ...
        if (dmgObject['crit'] === true && abils[settings['ability']]['crit effects'] === true) {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * (1 + calc_crit_damage(settings)));
        }

        // store bloat damages
        if (settings['ability'] === 'bloat - full damage') {
            settings['bloat damage'] = dmgObject;
        }
    }
    return dmgObject;
}
...
function calc_on_npc(settings, dmgObject) {
    for (let i=0; i<dmgObject['damage list'].length; i++) {
        ...
        if (settings['wilderness puzzlebox'] > 1) {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * (1 + 0.03 + settings['wilderness puzzlebox']));
        }
        ...
        if (settings['guardian triump'] === true && abils[settings['ability']]['ability type'] === 'basic') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.2);
        }
        ...
        dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * (1 + settings['nopenopenope']));
        ...
        if (settings['two-hand weapon'] === 'vanquish') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * (1 + 0.05 * settings['quest deaths']));
        }
        ...
        else if (settings['aura'] === 'berserk' && abils[settings['ability']]['main style'] === 'melee') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.1);
        }
        else if (settings['aura'] === 'reckless' && abils[settings['ability']]['main style'] === 'ranged') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.1);
        }
        ...
        if (settings['pocket'] === 'scrimshaw of elements' && abils[settings['ability']]['main style'] === 'magic') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.05);
        }
        else if (settings['pocket'] === 'superior scrimshaw of elements' && abils[settings['ability']]['main style'] === 'magic') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.0666);
        }
        
        // scrimshaw of cruelty
        if (settings['pocket'] === 'scrimshaw of cruelty' && abils[settings['ability']]['main style'] === 'ranged') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.05);
        }
        else if (settings['pocket'] === 'superior scrimshaw of cruelty' && abils[settings['ability']]['main style'] === 'ranged') {
            dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.0666);
        }
        ...
        if (abils[settings['ability']]['main style'] === 'magic' && settings['essence corruption'] >= 25) {
            dmgObject['damage list'][i] = dmgObject['damage list'][i] + settings['magic level'] + settings['essence corruption'];
        }
...
function calc_on_hit(settings, dmgObject) {
    [settings['min hit'], settings['var hit']] = set_min_var(settings, dmgObject);
    [settings['min hit'], settings['var hit']] = calc_additive_boosts(settings, dmgObject);
    [settings['min hit'], settings['var hit']] = calc_multiplicative_shared_buffs(settings, dmgObject);
    [settings['min hit'], settings['var hit']] = calc_multiplicative_pve_buffs(settings, dmgObject);
    [settings['min hit'], settings['var hit']] = calc_bonus_damage(settings, dmgObject);
...
function calc_damage_object(settings) { 
    ...
    for (let key in dmgObject) {
        ...
        // calc buffed AD
        dmgObject[key]['boosted AD'] = calc_base_ad(settings, dmgObject[key]);
        // calc on hit effects
        if (abils[settings['ability']]['on-hit effects']) {
            [dmgObject[key]['min hit'], dmgObject[key]['var hit']] = calc_on_hit(settings, dmgObject[key]);   
        }
        // roll damage
...
function calc_bolg(settings) {
    ...
    for (let key in bolg_damage_based) {
        ...
        for (let element in settings['bolg damage']) {
            ...
            [bolg_damage_based[key]['min hit'], bolg_damage_based[key]['var hit']] = calc_on_hit(settings, bolg_damage_based[key]);
...
function calc_bloat(settings) {
    ...
    for (let key in settings['bloat damage']) {
        for (dmg in settings['bloat damage'][key]) {
            bloat_dot[key].push(Math.floor(dmg/4));
        }
    }

    bloat_dot = calc_on_npc(settings, bloat_dot);
    bloat_dot = get_user_value(settings, bloat_dot);

    return 10*get_user_value(settings, bloat_dot);
}
...
function hit_damage_calculation(settings) {
    ...
    // handle bolg logic
    if (abils[settings['ability']]['main style'] === 'ranged' && settings['two-hand weapon'] === 'bow of the last guardian'
    ...
    if (settings['instability'] === true && abils[settings['ability']]['damage type']) {

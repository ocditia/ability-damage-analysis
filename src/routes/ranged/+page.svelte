<script>
	...
	import { settingsConfig, SETTINGS } from '$lib/ranged/settings';
	import Checkbox from '../../components/Settings/Checkbox.svelte';
	import Number from '../../components/Settings/Number.svelte';
	import Select from '../../components/Settings/Select.svelte';

	let damages = Object.fromEntries(
		Object.entries(abilities).map(([key, value]) => [key, { ...value, regular: 0, ss: 0 , ds: 0, dsss: 0}])
	);
	...
	function updateDamages() {
		...
		Object.entries(damages).forEach(([abilityKey, ability]) => {
			...
			damages[abilityKey].ds = ability.calc({...adaptedSettings});
            adaptedSettings['split soul'] = true;
            adaptedSettings['death swiftness'] = false;
			damages[abilityKey].dsss = ability.calc({...adaptedSettings});
		});
	}
</script>
						...
						<table>
							<thead>
								<tr>
                                    ...
                                    <th class="p-3 text-left">Death swiftness</th>
                                    <th class="p-3 text-left">split swift</th>
								</tr>
							</thead>
							<tbody>
								{#each Object.entries(damages) as [key, damage] (key)}
									<tr>
                                        ...
                                        <td class="p-3 text-left">{damage.ds}</td>
                                        <td class="p-3 text-left">{damage.dsss}</td>
									</tr>
								{/each}
							</tbody>
						</table>

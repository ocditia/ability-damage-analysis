...
function next_hit(settings) {
	...
	if ( settings[SETTINGS.TH] === SETTINGS.TH_VALUES.BOLG &&
		...
		if (settings[SETTINGS.BALANCE_BY_FORCE] === true && settings[SETTINGS.PERFECT_EQUILIBRIUM_STACKS] === 4) {
	...
	if (settings['ability'] === ABILITIES.SKELETON_WARRIOR_AUTO) {
		settings[SETTINGS.SKELETON_WARRIOR_RAGE_STACKS] = Math.min(25, settings[SETTINGS.SKELETON_WARRIOR_RAGE_STACKS]+1); 
	}

...
class AbilityDmg {
	calcAd(type, settings) {
		...
		if (type === '2h') {
			AD =
				Math.floor(2.5 * this.calcLevel(settings)) +
				Math.floor(1.25 * this.calcLevel(settings)) +
				Math.floor(14.4 * this.getTier('two-hand', settings) + 1.5 * this.calcBonus(settings));
		}
		...
		//dual-wield AD
			...
			const mhAbilityDmg =
				Math.floor(2.5 * this.calcLevel(settings)) +
				Math.floor(9.6 * this.getTier('main-hand', settings) + this.calcBonus(settings));
			let ohAbilityDmg = 0;
			if (type === 'Ms') {
				ohAbilityDmg = 0;
			} else if (type === 'Md') {
				ohAbilityDmg = Math.floor(
					0.5 *
						(Math.floor(2.5 * this.calcLevel(settings)) +
							Math.floor(
								9.6 * Math.floor(0.5 * this.getTier('defender', settings)) +
									this.calcBonus(settings)
							))
				);
			} else if (type === 'Dw') {
				ohAbilityDmg = Math.floor(
					0.5 *
						(Math.floor(2.5 * this.calcLevel(settings)) +
							Math.floor(9.6 * this.getTier('off-hand', settings) + this.calcBonus(settings)))
				);
			}
		...
		if (
			settings['inquisitor'] === true &&
			settings['two-handed weapon'] === 'inquisitor staff' &&
			type === '2h'
		) {
	...
	calcBonus(settings) {
		...
		return (
			construction['helmets'][settings['helmet']]['bonus'] +
	...
	calcLevel(settings) {
		...
		let percentBoost =
			construction['potions'][settings['potion']]['percent'] +
			construction['auras'][settings['aura']]['levels'];

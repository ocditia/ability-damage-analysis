	...
	if (settings['main_style'] === 'magic') {
		if (settings['weapon type'] === 'main-hand') {
			let AD_mh =
				...
				Math.floor(
					9.6 * Math.min(weapons[settings['main-hand weapon']]['tier'], spell_tier) +
						calc_bonus(settings)
				);
			...
			if (weapons[settings['off-hand weapon']]['weapon type'] === 'off-hand') {
				AD_oh = Math.floor(
					0.5 * Math.floor(2.5 * settings['magic level']) +
						Math.floor(
							9.6 * Math.min(weapons[settings['off-hand weapon']]['tier'], spell_tier) +
								calc_bonus(settings)
						)
				);
			}
			...
			base_AD =
				...
				Math.floor(
					14.4 * Math.min(weapons[settings['two-hand weapon']]['tier'], spell_tier) +
...
function ability_specific_effects(settings, dmgObject) {
	...
	if (settings['main_style'] === 'necromancy') {
		...
		if (settings['ability'] === 'skeleton warrior auto') {
			dmgObject[boosted_AD] = Math.floor(
				dmgObject[boosted_AD] * (1 + 0.03 * settings['skeleton warrior stacks'])
			);
		}
	}
}
...
function calc_additive_boosts(settings) {
	...
	if (void_chest_pieces.contains(settings['chest'])) {
		void_pieces += 1;
	}
	...
	if (void_legs_pieces.contains(settings['legs'])) {
		void_pieces += 1;
	}
	const void_hands_pieces = ['void knight gloves', 'superior void knight gloves'];
	if (void_hands_pieces.contains(settings['hands'])) {
		void_pieces += 1;
	}
	const void_shield_pieces = ['void knight deflector', 'superior void knight deflector'];
	if (void_shield_pieces.contains(settings['off-hand'] && 'weapon type' === 'main-hand')) {
		void_pieces += 1;
	}
	...
	// zerk necklace
	if (
		(weapons[settings['main-hand']]['category'] === 'obsidian' &&
			settings['weapon type'] === 'main-hand') ||
		(weapons[settings['two-hand']]['category'] === 'obsidian' &&
			settings['weapon type'] === 'two-hand')
	) {
		boost += 0.05;
	}
...
function calc_core(settings, dmgObject) {
	for (let i = 0; i < dmgObject['damage list'].length; i++) {
		...
		dmgObject['damage list'][i] = Math.floor(
			dmgObject['damage list'][i] * (1 + settings['berserkers fury'] / 100)
		);
		...
		if (dmgObject['crit'] === true && abils[settings['ability']]['crit effects'] === true) {
			dmgObject['damage list'][i] = Math.floor(
				dmgObject['damage list'][i] * (1 + calc_crit_damage(settings))
			);
		}
...
function calc_on_npc(settings, dmgObject) {
	for (let i = 0; i < dmgObject['damage list'].length; i++) {
		...
		if (ghost_hunter_pieces === 3 || ghost_hunter_pieces === 4) {
			dmgObject['damage list'][i] = Math.floor(dmgObject['damage list'][i] * 1.1);
		}
		...
		dmgObject['damage list'][i] = Math.floor(
			dmgObject['damage list'][i] * settings['anachronia slayer lodge buff']
		);
...
function calc_on_hit(settings, dmgObject) {
	...
	[settings['min hit'], settings['var hit']] = calc_multiplicative_shared_buffs(
		settings,
		dmgObject
	);
...
export default calc_damage_object;

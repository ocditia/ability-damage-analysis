...
function bolg_base_dirty(proccingHit, type, settings, numberOfHits) {
	...
	for (var hitsplat = 0; hitsplat < numberOfHits; hitsplat++) {
		...
		let fixed =
			Math.floor(AD * fixedPercent) +
			Math.floor(construction['abilities']['bolg proc']['fixed percent'] * proccingHit);
		let variable =
			Math.floor(AD * variablePercent) +
			Math.floor(construction['abilities']['bolg proc']['variable percent'] * proccingHit);
		...
		//add up damages
		damageObject['non-crit']['list'] = Helper.listAdder(
			damageObject['non-crit']['list'],
			splitSoul
		);
